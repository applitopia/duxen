...
test("SubEngine action creators", function() {
  const todos:Schema = {
    'todosFilter': {
      type: 'value',
      initValue: "Get milk",
  ...
  expect(insertAction).toEqual({"type": "DUXEN_INSERT", "collName": "todosApp.todos", "id": "id1", "doc": fromJS({"text": "Get tickets"})});

  const updateAction:UpdateAction = actionFactory.update("todos", "id1", ensure({"text": "Get tickets to concert"}));
  expect(updateAction).toEqual({"type": "DUXEN_UPDATE", "collName": "todosApp.todos", "id": "id1", "doc": fromJS({"text": "Get tickets to concert"})});

  const removeAction:RemoveAction = actionFactory.remove("todos", "id1");
  expect(removeAction).toEqual({"type": "DUXEN_REMOVE", "collName": "todosApp.todos", "id": "id1"});

  const resetAction:ResetAction = actionFactory.reset("todos");
  expect(resetAction).toEqual({"type": "DUXEN_RESET", "collName": "todosApp.todos"});

  const pauseAction:PauseAction = actionFactory.pause("todos");
  expect(pauseAction).toEqual({"type": "DUXEN_PAUSE", "collName": "todosApp.todos"});

  const resumeAction:ResumeAction = actionFactory.resume("todos");
  expect(resumeAction).toEqual({"type": "DUXEN_RESUME", "collName": "todosApp.todos"});

  const saveAction:SaveAction = actionFactory.save("todos");
  expect(saveAction).toEqual({"type": "DUXEN_SAVE", "collName": "todosApp.todos"});

  const restoreAction:RestoreAction = actionFactory.restore("todos");
  expect(restoreAction).toEqual({"type": "DUXEN_RESTORE", "collName": "todosApp.todos"});

  const saveOriginalsAction:SaveOriginalsAction = actionFactory.saveOriginals("todos");
  expect(saveOriginalsAction).toEqual({"type": "DUXEN_SAVE_ORIGINALS", "collName": "todosApp.todos"});

  const retrieveOriginalsAction:RetrieveOriginalsAction = actionFactory.retrieveOriginals("todos");
  expect(retrieveOriginalsAction).toEqual({"type": "DUXEN_RETRIEVE_ORIGINALS", "collName": "todosApp.todos"});
  ...
  expect(batchAction).toEqual({"type": "DUXEN_BATCH", "collName": "todosApp.todos", actions});
  ...
  expect(valueAction).toEqual({"type": "DUXEN_VALUE", "valueName": "todosApp.todosFilter", "value": "Get sugar"});

  const customAction:CustomAction = actionFactory.custom("customNextPage");
  expect(customAction).toEqual({"type": "todosApp.CUSTOM_NEXT_PAGE"});
